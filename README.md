...
*Lưu ý cho môi trường Windows:* Thư viện `mysqlclient` yêu cầu công cụ biên dịch C++ của Microsoft. Nếu gặp lỗi cài đặt, bạn có thể cài đặt package whl thích hợp hoặc chuyển sang dùng thư viện thay thế là `pymysql` (cần khai báo trong `__init__.py`).
